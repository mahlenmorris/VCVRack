...
struct TextInput {
  ...
  TextInput() : id{0} { decoder.provideDataBuffer(recvBuffer, recvBufferSize); }
...
struct TTY : Module {
  ...
  enum ParamId { CLEAR_PARAM, SAMPLE_PARAM, PAUSE_PARAM, PARAMS_LEN };
  enum InputId {
    V1_INPUT,
    TEXT1_INPUT,
    V2_INPUT,
  ...
  enum LightId { CLEAR_LIGHT, PAUSE_LIGHT, LIGHTS_LEN };

  enum TimeStamp { NONE, PATCH_TIME, LOCAL_TIME };
  ...
  // Don't only update every single tick. It spews out too many rows too
  // quickly. We let user control frequency. Small values let you see the Tipsy
  // protocol and find odd transients, larger values let you see the movement in
  // fewer lines.
  int tick_count = 0;
  ...
  TTY() {
    ...
    configParam(
        SAMPLE_PARAM, 1000.0f, 0.0f, 50.0f,
        "Number of milliseconds skipped between V1/V2/V3 logging attempts");
  ...
  void dataFromJson(json_t* rootJ) override {
    ...
    if (timestampJ)
      time_stamp_format =
          static_cast<TimeStamp>(json_integer_value(timestampJ));
  ...
  bool processTextInput(bool paused, TextInput* text_input) {
    if (inputs[text_input->id].isConnected()) {
      auto decoder_status =
          text_input->decoder.readFloat(inputs[text_input->id].getVoltage());
  ...
  std::string MakePrefix(const std::string& source) {
    ...
    switch (time_stamp_format) {
      ...
      case LOCAL_TIME: {
        ...
        char buffer[9];
        size_t s = std::strftime(buffer, sizeof(buffer), "%H:%M:%S",
                                 std::localtime(&t));
  ...
  void process(const ProcessArgs& args) override {
    ...
    if (!paused) {
      ...
      if (tick_count >
          params[SAMPLE_PARAM].getValue() * args.sampleRate / 1000) {
        tick_count = 0;
        if (inputs[V1_INPUT].isConnected()) {
          ...
          if (!float_equal(v1, previous_v1)) {
            ...
            // Hmmmm; should I be comparing the string values instead? It would
            // be odd to see the same string twice on a tightly changing
            // value....
    ...
    clear_command_received |= processTextInput(paused, &ti1);
    clear_command_received |= processTextInput(paused, &ti2);
    clear_command_received |= processTextInput(paused, &ti3);
    ...
    bool clear =
        clear_command_received || params[CLEAR_PARAM].getValue() > 0.1f;
...
struct TTYTextField : STTextField {
  ...
  void set_visible_lines(int visible_lines) {
    ...
    // TODO: maybe this has to do with the 2nd arg to extended.Initialize()
    // below?
    large_text_mode = visible_lines <= 6;
    // At fontsize 12, it's 28 rows.
    // When only a couple of lines high, scrolling before getting to top or
    // bottom is frustrating.
  ...
  void step() override {
    ...
    if (module) {
      ...
      if (color_scheme != module->screen_colors || module->editor_refresh) {
        // Note: this doesn't actully care about editor_refresh. But this
        // cleared up a bug about duplicated windows not keeping the same color.
...
struct TTYWidget : ModuleWidget {
  ...
  TTYWidget(TTY* module) {
    ...
    addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(8.938, 22.329)),
                                                 module, TTY::SAMPLE_PARAM));
    addParam(
        createLightParamCentered<VCVLightLatch<MediumSimpleLight<WhiteLight>>>(
            mm2px(Vec(8.938, 68.2)), module, TTY::PAUSE_PARAM,
            TTY::PAUSE_LIGHT));
    addParam(
        createLightParamCentered<VCVLightButton<MediumSimpleLight<WhiteLight>>>(
            mm2px(Vec(8.938, 79.3)), module, TTY::CLEAR_PARAM,
            TTY::CLEAR_LIGHT));

    addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(8.938, 34.0)),
                                                   module, TTY::V1_INPUT));
    addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(8.938, 45.0)),
                                                   module, TTY::V2_INPUT));
    addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(8.938, 56.0)),
                                                   module, TTY::V3_INPUT));
    addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(8.938, 92.0)),
                                                   module, TTY::TEXT1_INPUT));
    addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(8.938, 105.0)),
                                                   module, TTY::TEXT2_INPUT));
    addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(8.938, 118.0)),
                                                   module, TTY::TEXT3_INPUT));
    ...
    textDisplay->box.size.x =
        box.size.x - RACK_GRID_WIDTH * NON_SCREEN_WIDTH - mm2px(CONTROL_WIDTH);
    ...
    // Resize bar on right.
    rightHandle =
        new StochasticTelegraph::STResizeHandle<TTY, TTYUndoRedoAction>(true, 4,
                                                                        300);
  ...
  void appendContextMenu(Menu* menu) override {
    ...
    MenuItem* time_menu =
        createSubmenuItem("Preface lines with timestamp?", "", [=](Menu* menu) {
    ...
    menu->addChild(
        createBoolPtrMenuItem("Keep recent output when patch is saved", "",
    ...
    std::pair<std::string, std::string> fonts[] = {
        ...
        {"Veteran Typewriter (TTY-ish font)",
         "fonts/veteran-typewriter.regular.ttf"},

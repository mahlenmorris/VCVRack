...
float RandomDistribution::next() const {
  ...
  // Value morphing: interpolates Inverse-CDF equivalents instead of probability
  // fractions.
  if (dist_clamped <= 1.0f) {
    float v1 =
        (random::uniform() + random::uniform() + random::uniform()) / 3.0f;
    ...
    float v3 = static_cast<float>(
        (sign * std::pow(std::abs(temp_val), U_DIST_EXPONENT)) + 0.5);
  ...
  if (bias_c != 1.0f) {
    result_0_to_1 =
        result_0_to_1 / (result_0_to_1 + (1.0f - result_0_to_1) * bias_c);
  }
  ...
  if (pdf_section == LEFT) {
    ...
    result_0_to_1 =
        (result_0_to_1 - 0.5f) * 2.0f; // Shift left and scale to [0, 1].
  }

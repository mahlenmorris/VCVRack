...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    TOK_QUESTION = 30,             // "?"
    TOK_COLON = 31,                // ":"
    TOK_IDENTIFIER = 32,           // "identifier"
    TOK_QUOTED_STRING = 33,        // "quoted_string"
    TOK_NUMBER = 34,               // "number"
    TOK_NOTE = 35,                 // "note"
    TOK_ONEARGFUNC = 36,           // "oneargfunc"
    TOK_TWOARGFUNC = 37,           // "twoargfunc"
    TOK_COMPARISON = 38,           // "comparison"
    TOK_NEG = 39                   // NEG
      };
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_QUOTED_STRING = 33,                    // "quoted_string"
        S_NUMBER = 34,                           // "number"
        S_NOTE = 35,                             // "note"
        S_ONEARGFUNC = 36,                       // "oneargfunc"
        S_TWOARGFUNC = 37,                       // "twoargfunc"
        S_COMPARISON = 38,                       // "comparison"
        S_NEG = 39,                              // NEG
        S_YYACCEPT = 40,                         // $accept
        S_meta_start = 41,                       // meta_start
        S_circle_list = 42,                      // circle_list
        S_circle = 43,                           // circle
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_circle: // circle
        value.move< Circle > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Assignments&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Assignments& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Circle&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Circle& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, CircleList&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const CircleList& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_circle: // circle
        value.copy< Circle > (YY_MOVE (that.value));
        break;
  ...
  Parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_circle: // circle
        value.move< Circle > (YY_MOVE (s.value));
        break;

...
class ExpressionFactory {
 public:
  ...
  Expression OneArgFunc(const std::string& func_name, const Expression& arg1);
  Expression OnePortFunc(const std::string& func_name, const std::string& port1,
                         Driver* driver);
  Expression TwoArgFunc(const std::string& func_name, const Expression& arg1,
                        const Expression& arg2);
  Expression TernaryFunc(const Expression& condition, const Expression& if_true,
                         const Expression& if_false);
  Expression CreateBinOp(const Expression& lhs, const std::string& op_string,
                         const Expression& rhs);
  Expression ArrayVariable(const std::string& array_name,
                           const Expression& arg1, Driver* driver);
 ...
 private:
  static std::unordered_map<std::string, Expression::Operation>
      string_to_operation;
};
...
struct ExpressionList {
  ...
  ExpressionList() { is_strings = false; }
...
struct Line {
  ...
  // out of the names; string and float variables live in completely separate
  // namespaces, which the parser has sufficient information to determine. I
  // hope.
  ...
  static Line ElseIf(const Expression& bool_expr, const Statements& state1);
  ...
  static Line IfThen(const Expression& bool_expr, const Statements& then_state,
